...
public class MTF_NineTailedFox extends Human{
    public MTF_NineTailedFox(Player player, RoleTemplate roleTemplate) {
        super(player, roleTemplate);
        update();

        giveItems();
        ArmorManager.getInstance().createMTFSuit(player);
    }
